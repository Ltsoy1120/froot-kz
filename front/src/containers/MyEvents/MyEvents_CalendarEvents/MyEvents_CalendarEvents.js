...
const MyEvents_CalendarEvents = () => {
    ...
    allEvents = events.map((el, i) => {
      ...
      return (
        <div key={i} className={"MyEvents_CalendarEvents__item"}>
          ...
          <p className={"MyEvents_CalendarEvents__text"}>
            <b>Дата встречи:</b> {date.format("DD-MM-YYYY")}
          </p>
          <p className={"MyEvents_CalendarEvents__text"}>
            <b>C:</b> {el.from}
          </p>
          ...
          {el.file !== "null" ? (
            <a
              target={"blanc"}
              className={"MyEvents_CalendarEvents__link"}
              href={`${apiURL}/uploads/${el.file}`}
            >
              Файл
            </a>
          ) : null}
          <div className={"MyEvents_CalendarEvents__participantsBlock"}>
            <p className={"MyEvents_CalendarEvents__title"}>
              Участники встречи:
            </p>
            {el.participants.map((el, i) => {
              return (
                <div
                  key={i}
                  className={"MyEvents_CalendarEvents__participantItem"}
                >
                  <p>
          ...
          {thisUser[0].accepted === null ? (
            <div className={"MyEvents_CalendarEvents__btnBlock"}>
              <ButtonPink
                onClickHandler={() => {
                  accept(el._id);
                }}
  ...
  return <div className={"MyEvents_CalendarEvents"}>{allEvents}</div>;
};
