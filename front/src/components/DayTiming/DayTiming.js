...
const DayTiming = props => {
  ...
  const pickTheRange = (event, i) => {
    ...
    if (!firstClick) {
      ...
      setScale(scaleCopy);
      setChosenTime(
        `С ${event.target.textContent} до ${parseMyTime(
      ...
      if (firstClick.index < i) {
        ...
        for (let k = allCells.length - 1; k >= 0; k--) {
          if (firstClick.index > k && k >= i) {
            if (
              allCells[k].style.background === "rgb(255, 255, 255)" ||
              !allCells[k].style.background
            ) {
              scaleCopy.push(allCells[k].textContent);
              allCells[k].style.background = color;
              setChosenTime(
                `С ${allCells[k].textContent} до ${parseMyTime(
                  firstClick.time
                )}`
              );
            } else {
  ...
  if (activeTimes) {
    timeTable = (
      <div>
        <h2 className="DayTiming__table__title">
          Расписание резервов на {month} {day}, {year}
        </h2>
          ...
          {timePeriods.map((el, i) => {
            ...
            if (index > -1) {
              ...
              participants = activeTimes[index].participants.map((el, i) => {
                return (
                  <div
                    key={i}
                    className={"DayTiming__modalInfo__participantsBlock"}
                  >
                    <p
                      className={"DayTiming__modalInfo__participantsName"}
                      key={i}
                    >
                      {el.name}
                    </p>
                    <p className={"DayTiming__modalInfo__participantsStatus"}>
                      {el.accepted === null
                        ? "Еще не подтвердил"
                        : el.accepted
                        ? "Подтвердил"
            ...
            return (
              <CellsLine
                ...
                timeContent={el}
                pickRange={event => {
                  pickTheRange(event, i);
                }}
                titleInModal={index > -1 ? activeTimes[index].title : ""}
                title={
                  index > -1
                    ? activeTimes[index].scale[0] === el
                      ? activeTimes[index].title
                      : ""
                    : ""
                }
                isActiveTime={index > -1}
                creator={
                  index > -1 && activeTimes[index].user
                    ? `${activeTimes[index].user.name} ${activeTimes[index].user.surname}`
                    : ""
                }
                deleteReserve={
                  index > -1
                    ? () => {
                        deleteReserve(activeTimes[index]._id);
                      }
                    : null
                }
                ...
                getFile={() => {
                  download(
                    `${apiURL}/uploads/${activeTimes[index].file}`,
                    activeTimes[index].file
                  );
                }}
  ...
  let displayWorkers = chosenWorkers.map((el, i) => {
    return (
      <div key={i} className="DayTiming__workerName">
        <p className="DayTiming__workerName--text">
          {el.name} {el.surname} - {el.position}
        </p>
        <div
          onClick={() => {
  ...
  const submitEvent = async event => {
    ...
    let obj = {
      ...
      date: fullDate,
      from: secondClick
        ? firstClick.index < secondClick.index
          ? firstClick.time
          : secondClick.time
        : firstClick.time,
      to: secondClick
        ? firstClick.index > secondClick.index
          ? parseMyTime(firstClick.time)
          : parseMyTime(secondClick.time)
        : parseMyTime(firstClick.time)
    };
  ...
  return (
    <div className="DayTiming">
      ...
      <div className="DayTiming__footer">
        {chosenTime ? (
          <p className="DayTiming__footer--text">{chosenTime}</p>
        ) : (
        ...
        >
          <div className="DayTiming__footer--form">
            ...
            <input
              ...
              name="title"
              required
              className="DayTiming__footer--input"
              type="text"
            />
            ...
            <textarea
              ...
              name="description"
              required
              className="DayTiming__footer--input DayTiming__footer--textarea"
              type="text"
            />
            ...
            <div className="DayTiming__form--fileBlock">
              <input
                ...
                name={"file"}
                className="DayTiming__form--hideInput"
                ref={fileLoader}
                type="file"
              />
              <div
                className="DayTiming__form--chooseFileBtn"
              ...
              {file ? (
                <div className="DayTiming__form--fileBlock--toggled">
                  <p className="DayTiming__form--text DayTiming__form--text--shorten">
                    {file.name}
                  </p>
                  <div
                    onClick={deleteChosenFile}
                    className="DayTiming__workerName--deleteIcon"
                  />
                </div>
              ) : (
            ...
            <select
              ref={selectRef}
              defaultValue={"Выберите участников"}
              onChange={event => {
                pickAWorker(event);
              }}
              required
          ...
          <div className="DayTiming__footer--bottom">
            <button
              disabled={
                !scale.length > 0 ||
                !Object.keys(restOfState).length > 0 ||
                !chosenWorkers.length > 0
              }
              className="DayTiming__btn"
            >
              Зарезервировать
            </button>
          </div>
